...
func GetRegionalEndpoint(regionName *string) (*string, error) {
	...
	if !isRegionValid {
		return nil, custom_error.NewUnprocessableEntityError(fmt.Errorf("Region " + *regionName + " not valid"))
	}
	...
	if ok {
		...
		return &i, custom_error.NewNotFoundError(fmt.Errorf("No such endpoint for regionName " + *regionName))
	}
}
